...
use std::{fs::Permissions, io::Error, os::unix::prelude::PermissionsExt, path::{Path, PathBuf}, str::FromStr};

use crate::uevent::{Action, UEvent};
use nix::{sys::stat::makedev, unistd::{Gid, Uid}};
use nix::sys::stat::{mknod, mode_t, Mode, SFlag};

// Functions to handle UEvent.

pub fn handle_uevent<P>(event:&UEvent) -> Result<(),std::io::Error> where P: pal::permissions::DefaultAttributes {
    match event.action {
...
pub fn handle_add<P>(event : &UEvent) -> Result<(),std::io::Error> where P : pal::permissions::DefaultAttributes{
    ...
    match  event.maybe_subsystem.as_ref().unwrap().as_str() {
        "block" => {
            ...
            create_device(&device_path, attrs.mode, attrs.owner, attrs.group, event.get_dev_type(), event.maybe_major.unwrap(), event.maybe_minor.unwrap())?;
            if let Some(link) = link_by_name {
                create_links(&device_path, &vec![&link], attrs.owner, attrs.group, attrs.mode )?;
            }
...
fn create_dir_if_needed(dev_path:&Path, uid: libc::uid_t, gid: libc::gid_t, mode: mode_t) -> Result<(),std::io::Error>{
    for p in dev_path.parent().unwrap().ancestors() {
        if !p.exists() {
            //create the directory
            std::fs::create_dir(p)?;
            std::fs::set_permissions(p, Permissions::from_mode(mode))?;            
            nix::unistd::chown(p, Some(Uid::from_raw(uid)), Some(Gid::from_raw(gid))).map_err(|_e|std::io::Error::new(std::io::ErrorKind::PermissionDenied, "Unable to change permission"))?;
        }
    }
    Ok(())
}

fn create_device(dev_path:&Path, mode: mode_t, uid: libc::uid_t, gid: libc::gid_t, kind:SFlag, major: u64, minor: u64) -> Result<(),std::io::Error> {
    create_dir_if_needed(dev_path, uid, gid, mode)?;
    if !dev_path.exists() {
        mknod(dev_path, kind, Mode::from_bits(mode).unwrap() , makedev(major, minor))
            .map_err(|_e|std::io::Error::new(std::io::ErrorKind::Unsupported, "Unable to change permission"))
    } else {
        Ok(())
...
fn create_links(dev_path:&Path, links: &Vec<&Path>, uid : libc::uid_t, gid: libc::gid_t, mode: libc::mode_t ) -> Result<(),std::io::Error> {
    for link in links {
        if !link.exists() {
